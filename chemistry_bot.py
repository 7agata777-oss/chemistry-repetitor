...
def load_progress():
    """Загружает все данные из базы в словарь {user_id: {topic_id: {attempts, best_percent}}}"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
...
def save_progress(user_id, topic_id, attempts, best_percent):
    """Вставляет или обновляет запись о прогрессе"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("""
...
# Глобальный словарь прогресса загружается из БД при старте
...
async def start_test(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Отправляем новое сообщение с вопросом и кнопками
...
async def process_answer(callback: types.CallbackQuery, state: FSMContext):
    ...
    if idx < len(TOPICS[topic_id]["questions"]):
        ...
        # Показываем результат предыдущего вопроса и следующий вопрос в новом сообщении
        await callback.message.edit_text(
